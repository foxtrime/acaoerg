...
function exibirOnClick(vol,num,id){
    ...
    for(i = 1; i <= 6; i++){    //adicionar uma unidade para cada edicao q for adicionada / alterar height e revista css
    ...
    td.style.borderRightColor = "skyblue";
    for(i = 1; i <= 6; i++){    //adicionar uma unidade para cada edicao q for adicionada
      if(i != id){
        var td  = document.getElementById(i);
...
function pop(idArtigo){
  window.open("edicoes/resumo.php?idArtigo="+idArtigo,"","width=670px, height=515px, top=40px, left=40px, scrollbars=yes, resizable=no, titlebar=no, location=no, status=no");
}
